...
class Transaction
{
    ...
    protected $r;
    protected $s;
    public function __construct(\phpEther\Account $from = NULL, string $to = NULL, int $value = NULL, string $data = NULL, int $nonce = NULL, int $gasPrice = NULL, int $gasLimit = NULL , $web3=NULL )
    {
		$this->account = $from;
	...
	public function setGasLimit($gasLimit= NULL){
		...
		$this->gasLimit = $est instanceof \BitWasp\Buffertools\Buffer?$est:Buffer::hex(Hex::cleanPrefix($est));
		return $this;
	}
	...
	public function send(){  
		...
		if(!is_null($this->account->password)){
			...
			return Hex::getHex($this->hash);
		}
		
		if(!is_null($this->account->privateKey)){
			$RawTx = $this->sign()->getRaw();
			$hash = $this->web3->eth->sendRawTransaction(Hex::getHex($RawTx));
			$this->hash = Buffer::hex(Hex::cleanPrefix($hash));
			return Hex::getHex($this->hash);
		}
